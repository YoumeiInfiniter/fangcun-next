...
class ReleaseBuilderTests(unittest.TestCase):
    ...
    def test_build_package_excludes_legacy_and_private_dirs(self):
        ...
        with zipfile.ZipFile(zip_path) as zf:
            names = zf.namelist()
        joined = "\n".join(names)
        self.assertIn("fangcun-next-0.2.1/SKILL.md", joined)
        self.assertIn("fangcun-next-0.2.1/scripts/project_cli.py", joined)
        self.assertIn("fangcun-next-0.2.1/references/schemas/project-config.schema.json", joined)
