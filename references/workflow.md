...
汇总（不逐集轰炸）。新项目必须先生成至少三种可执行 `capacity_plan` 方案，
由编剧明确选择集数、时长窗口以及保留/压缩/延期/省略/接受溢出的事件取舍，
再以当前 `forecast` 与 `episode_outline` 哈希绑定的版本确认集纲。无风险时系统
记录 `not_applicable`；历史项目的 `capacity_decisions.jsonl` 只读兼容，不会冒充
新的容量计划。剧本阶段即使超时也只提示，不自动回退上游。
...
剧本：Writer 只执行当前不可变上下文派生的 `episode_execution_brief`；建立认知链与
