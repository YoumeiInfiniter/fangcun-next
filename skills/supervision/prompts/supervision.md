...
- 💡 **付费点体量参考（柔性建议，编剧有最终裁量权）**：
  - 若配置集数N ≤ 10集 → 建议两卡点模式（前50%免费/后50%付费），但编剧指定位置时以编剧为准
  - 若配置集数N ≤ 15集 → 可参考前8集免费/第9集付费
  - 若集纲中出现超出总集数范围的付费点编号（如10集配置出现"第15集卡点"）→ ⚠️ 提示冲突，附带建议修正值，但不强制
  - 若编剧指定了卡点位置（如"卡在第8集"）→ 以编剧为准，系统的建议仅作为参考
  - 参考公式（非强制）：第一付费点 ≈ ⌈N × 0.5⌉；第二付费点 ≈ ⌈N × 0.75⌉
